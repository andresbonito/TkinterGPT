...
def criar_aba_1(notebook, criar_pergunta):
    ...
    tk.Radiobutton(tab1, text='Dificil', value='Dificil', variable=dificuldade_var).grid(row=2, column=3, sticky='W', padx=5, pady=5)

    # linha 3
    tk.Label(tab1, text='Pergunta exemplo:').grid(row=3, column=0, sticky='W', padx=5, pady=5)

    # linha 4
    pergunta_exemplo_entry = tk.Text(tab1)
    pergunta_exemplo_entry.configure(height=20)
    pergunta_exemplo_entry.grid(row=4, column=0, columnspan=4, sticky='WE', padx=5, pady=5)
    ...
    def executar():
        ...
        resposta = criar_pergunta(
            ...
            tipo_var.get(),
            dificuldade_var.get(),
            pergunta_exemplo_entry.delete('1.0', 'end')
        )
...
def criar_aba_2(notebook, responder_pergunta):
    ...
    # linha 0
    tk.Label(tab2, text='Escreva aqui a sua pergunta:').grid(row=0, column=0, sticky='W', padx=5, pady=5)
    ...
    # linha 2
    tk.Label(tab2, text='Resposta do ChatGPT:').grid(row=2, column=0, sticky='W', padx=5, pady=0)
    ...
    tk.Button(tab2, text='Ok', command=executar).grid(row=4, column=0, sticky='WE', padx=5, pady=5)
...
def criar_tela(criar_pergunta, responder_pergunta):
    window = tk.Tk()
    window.title('ChatGPT - Gerador/Corretor de perguntas')
    window.minsize(900, 600)
    notebook = ttk.Notebook(window, padding=10)
    tab1 = criar_aba_1(notebook, criar_pergunta)
